...
public class Fibonacci {
    ...
    public static int atIndex(int i) throws IllegalArgumentException {
    	if (i < 0) {
    		throw new IllegalArgumentException("Argument 'i' cannot be negative.");
    	}
        int a = 1;
        int b = 1;
        for(int j = 0; j < i; j++) {
    ...
    public static int[] slice(int start, int end) throws IllegalArgumentException {
    	if(start < 0 || end < 0 || end < start) {
    		throw new IllegalArgumentException("Argument 'start' and 'end' must be non-negative, and 'end' must be greater than 'start'.");
    	}
